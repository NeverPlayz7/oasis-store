...
let STATE = {
    user: null, cart: [],
    products: [
        {id: 1, name: "Oasis Watch X", price: 299, img: "https://picsum.photos/400/500?random=50"},
        {id: 2, name: "Stealth Buds", price: 149, img: "https://picsum.photos/400/500?random=51"},
        {id: 3, name: "Void Runner", price: 599, img: "https://picsum.photos/400/500?random=52"},
        {id: 4, name: "Cyber Pack", price: 199, img: "https://picsum.photos/400/500?random=53"}
    ]
};
...
function renderProducts() {
    document.getElementById('productGrid').innerHTML = STATE.products.map(p => `
        <div class="product-card p-6 group">
            <img src="${p.img}" class="rounded-[40px] mb-8 w-full h-80 object-cover group-hover:scale-105 transition duration-700">
...
function updateCartUI() {
    ...
    document.getElementById('cartItems').innerHTML = STATE.cart.map((item, i) => {
        ...
        return `<div class="bg-white/5 p-6 rounded-[30px] flex justify-between items-center">
            <div><p class="text-[10px] text-cyan-500 font-bold uppercase">${item.name}</p><b class="text-2xl">$${item.price}</b></div>
...
window.toggleHistory = async () => {
    ...
    document.getElementById('historyItems').innerHTML = snap.docs.map(doc => `
        <div class="bg-white/5 p-8 rounded-[40px]">
            <p class="text-[9px] font-black text-cyan-500 mb-2 uppercase">#${doc.id.slice(0,8)}</p>
            <p class="text-4xl font-black italic">$${doc.data().total}</p>
        </div>
    `).join('');
};

window.toggleAdmin = async (s) => {
    ...
    if(s) {
        ...
        document.getElementById('adminOrders').innerHTML = snap.docs.map(doc => `
            <div class="bg-white/5 p-8 rounded-[40px] flex justify-between">
                <div><p class="font-bold uppercase">${doc.data().email}</p></div>
                <div class="text-3xl font-black text-cyan-500">$${doc.data().total}</div>
            </div>
        `).join('');
    }
};
